...
async function profanityChecker(text) {
  ...
  if (response.data.data.detection_details.resulting_categories.length === 0)
    return false;
  return true;
}
// => End all function
...
app.post("/api/developers/register", async (req, res) => {
  ...
  const validateData = Joi.object({
    username: Joi.string().required().external(cek_add_user).messages({
      "string.empty": "something wrong Please check again",
      "any.required": "Please check value",
    }),
    email: Joi.string().email().required().external(cek_user_email).messages({
      "string.empty": "something wrong Please check again",
      "any.required": "check value",
  ...
  if (!hasUppercase || !hasNumber) {
    return res
      .status(400)
      .send("Password must contain capital letters and numbers");
  } else {
    result = await db.Developers.findAll();
    let id =
      formatname + (parseInt(result.length) + 1).toString().padStart(3, "0");
    let nohp = formatnumber + phone;
    await db.Developers.create({
      developer_id: id,
      username: username,
      email: email,
      password: password,
...
app.post("/api/developers/login", async (req, res) => {
  ...
  const validateData = Joi.object({
    ...
    password: Joi.string().required().messages({
      "string.empty": "check value password",
      "any.required": "check value",
    }),
  });
  try {
...
app.post("/api/developers/topup", async (req, res) => {
  ...
  try {
    ...
    return res.status(401).send({ message: "Invalid JWT Key" });
  }

  const validateData = Joi.object({
    saldo: Joi.string().required().messages({
      "string.empty": "Please input saldo",
      "any.required": "check value",
    }),
  });
  try {
  ...
  if (parseInt(saldo) < 1) {
    return res.status(400).send({ message: "Invalid value minimum 1" });
  }
  ...
  await db.Developers.update(
    { saldo: sum_saldo },
    { where: { developer_id: validation_token.developer_id } }
  );
  return res.status(200).send({ Saldo: sum_saldo });
});
...
app.post("/api/developers/recharge", async (req, res) => {
  ...
  const validateData = Joi.object({
    api_hit: Joi.string().required().messages({
      "string.empty": "Please input api hit",
      "any.required": "check value",
    }),
  });
  try {
  ...
  await db.Developers.update(
    { api_hit: sum_api_hit, saldo: count_balance },
    { where: { developer_id: validation_token.developer_id } }
  );
  return res
    .status(200)
    .send({ "Total Api Hit": sum_api_hit, "Balance Count": count_balance });
});
...
app.post("/api/developers/add/user", async (req, res) => {
  ...
  const validateData = Joi.object({
    group_id: Joi.string().required().external(cek_develop).messages({
      "string.empty": "something wrong Please check again",
      "any.required": "Please check value",
    }),
    user_id: Joi.string().required().messages({
      "string.empty": "Please input user id",
      "any.required": "check value",
    }),
  });
  try {
  ...
  if (temp == false) {
    return res.status(400).send({ messages: "Api hit is not enough!" });
  }
...
app.post(
  ...
  async function (req, res) {
    ...
    if ((await profanityChecker(await translateText(group_name))) === true)
      return res
        .status(400)
        .send({ message: "Group_name contains offensive word" });
    if (
      (await profanityChecker(await translateText(group_description))) === true
    )
      return res
        .status(400)
        .send({ message: "Group_description contains offensive word" });

    const groups = await db.Groups.findAll();
    let group_id;
    if (groups.length == 0) {
      group_id =
        "GRP" + (parseInt(groups.length) + 1).toString().padStart(3, "0");
    } else {
      group_id =
        "GRP" +
        (parseInt(groups[groups.length - 1].group_id.substring(3)) + 1)
          .toString()
          .padStart(3, "0");
    }
    ...
    const developer = await db.Developers.findByPk(
      validation_token.developer_id
    );
...
app.post("/api/thread", cekToken, async (req, res) => {
  ...
  if (
    (await profanityChecker(await translateText(thread_description))) === true
  ) {
    return res.status(400).send({
...
app.post("/api/post", uploadFile.single("post_file"), async (req, res) => {
  ...
  if (!thread_id)
    return res.status(400).send({ message: "Thread_id cannot be empty!" });

  let thread = await db.Threads.findByPk(thread_id);
  if (!thread) return res.status(404).send({ message: "Thread_id not found!" });
  let cekGroup = await db.Groups.findByPk(thread.group_id);
  if (cekGroup.developer_id !== validation_token.developer_id)
    return res
      .status(400)
      .send({ message: "Developer cannot create this post!" });
  if (!user_id)
  ...
  if (!member)
    return res.status(400).send({ message: "User cannot create this post!" });
  if (!post_name)
    return res.status(400).send({ message: "Post_name cannot be empty!" });

  let jml = await db.Posts.findAll();
  let id;
  if (jml.length == 0) {
    id = "POS" + (parseInt(jml.length) + 1).toString().padStart(3, "0");
  } else {
    id =
      "POS" +
      (parseInt(jml[jml.length - 1].post_id.substring(3)) + 1)
        .toString()
        .padStart(3, "0");
  }
  ...
  if ((await profanityChecker(await translateText(post_name))) === true)
    return res
      .status(400)
      .send({ message: "Post_name contains offensive word" });
  if ((await profanityChecker(await translateText(post_description))) === true)
    return res
      .status(400)
      .send({ message: "Post_description contains offensive word" });
  let file = "";
...
app.put(
  ...
  async (req, res) => {
    ...
    if (!post_id)
      return res.status(400).send({ message: "Post_id cannot be empty!" });
    ...
    if (cekGroup.developer_id !== validation_token.developer_id)
      return res
        .status(400)
        .send({ message: "Developer cannot edit this post!" });
    let member = await db.Group_members.findOne({
      where: {
        group_id: cekGroup.group_id,
        user_id: user_id,
    ...
    if (!member)
      return res.status(400).send({ message: "User cannot edit this post!" });

    if (!post_description && !req.file && !post_name && !action)
      return res
        .status(400)
        .send({ message: "At least 1 field must be filled!" });
    if (post_name == "")
      return res
        .status(400)
        .send({ message: "Post_name cannot be empty string!" });
    ...
    if (post_description) {
      if (
        (await profanityChecker(await translateText(post_description))) === true
      )
        return res
          .status(400)
          .send({ message: "Post_description contains offensive word" });
...
app.delete("/api/post/:post_id", async (req, res) => {
  ...
  if (cekGroup.developer_id !== validation_token.developer_id)
    return res
      .status(400)
      .send({ message: "Developer cannot delete this post!" });
...
app.post("/api/comment", async (req, res) => {
  ...
  if (cekGroup.developer_id !== validation_token.developer_id)
    return res
      .status(400)
      .send({ message: "Developer cannot create this comment!" });
  let member = await db.Group_members.findOne({
    where: {
      group_id: cekGroup.group_id,
      user_id: user_id,
  ...
  if (!member)
    return res
      .status(400)
      .send({ message: "User cannot create this comment!" });
  if (!comment)
    return res.status(400).send({ message: "Comment cannot be empty!" });

  if ((await profanityChecker(await translateText(comment))) === true)
    return res.status(400).send({ message: "Comment contains offensive word" });
...
app.delete("/api/comment/:comment_id", async function (req, res) {
  ...
  if (!comment)
    return res.status(404).send({ message: "Comment_id not found! " });
  ...
  if (cekGroup.developer_id !== validation_token.developer_id)
    return res
      .status(400)
      .send({ message: "Developer cannot delete this comment!" });
  //API HIT 2
  const temp = await bayar_api_hit(validation_token.developer_id, 2);
  if (temp == false) {
    return res.status(400).send({ messages: "Api hit is not enough!" });
...
app.put("/api/comment/:comment_id", cekToken, async (req, res) => {
  ...
  if (comment) {
    if ((await profanityChecker(await translateText(comment))) === true)
      return res
        .status(400)
        .send({ message: "Comment contains offensive word" });
